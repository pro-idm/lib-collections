...
// Параметры:
// 	Коллекция - 
// 	Выражение - Строка - выполняемое для каждого элемента коллекциии выражение
// 	Контекст - Неопределено - Описание
Процедура ДляКаждого(Знач Коллекция, Знач Выражение, Знач Контекст = Неопределено) Экспорт
	...
	Если Коллекция.Количество() > 0 И Не ПустаяСтрока(Выражение) Тогда
		Шаблон = ШаблонДляКаждого(ТипКоллекции);
...
Процедура ПроверкаПараметров(Знач ТипКоллекции, Выражение, Контекст)
	Если ТипКоллекции <> Тип("Массив") И ТипКоллекции <> Тип("Структура") И ТипКоллекции <> Тип("Соответствие") 
			И ТипКоллекции <> Тип("СписокЗначений") И ТипКоллекции <> Тип("ТаблицаЗначений") Тогда
...
Функция ШаблонДляКаждого(ТипКоллекции)
	Если ТипКоллекции = Тип("Структура") Или ТипКоллекции = Тип("Соответствие") Тогда
		Шаблон = 
			...
			|
			|Индекс = 0;
			|Для Каждого Элемент Из Л_Коллекция Цикл
			|	%Выражение%
			|	Индекс = Индекс + 1;
			|КонецЦикла;";
	Иначе
